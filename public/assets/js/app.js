// Removed imports for direct browser usage
...
(function (global, factory) {
  ...
  onDOMContentLoaded(() => {
    ...
    document.addEventListener('mozfullscreenchange', syncFullscreenIcons)
    document.addEventListener('msfullscreenchange', syncFullscreenIcons)

    document.addEventListener('keydown', event => {
      if (event.key === 'F11') {
        event.preventDefault()
        const button = document.querySelector(SELECTOR_FULLSCREEN_TOGGLE)
        if (button) {
          const data = new FullScreen(button, undefined)
          data.toggleFullScreen()
        }
