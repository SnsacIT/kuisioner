...
class ReportController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        $data = DB::select("SELECT
                ...
                COALESCE(MAX(CASE WHEN p.id = 18 THEN kji.jawaban END),'') AS `Siapa saja yang Anda ketahui terlibat pada kejadian tersebut?`,
                COALESCE(MAX(CASE WHEN p.id = 18 THEN kji.description END),'') AS `Siapa saja yang Anda ketahui terlibat pada kejadian tersebut? (desc)`,
